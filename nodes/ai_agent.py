...
system_prompt : optional. Sets the assistant's role/behaviour. If structured
                output is enabled, you can write [fieldname] instructions.
user_prompt   : the message sent to the model. Supports {{ }} references.
output_shape  : optional (legacy). Paste a JSON example when structured is OFF.
structured    : if true, use the output_variables list below.
output_variables : list of variable names to extract from AI output.
...
class AINode(Node):
    ...
    def run(self, items):
        ...
        for item in items:
            ...
            if not isinstance(parsed, dict):
                out.append({"json": {**j, "error": "AI node: reply was not a JSON object",
    ...
    def _call(self, api_key, body):
        ...
        # usage.total_tokens -- this node never reported tokens before, so
        # DeepSeek calls never showed up in the run log's token count
        tokens_used = (payload.get("usage") or {}).get("total_tokens", 0)
